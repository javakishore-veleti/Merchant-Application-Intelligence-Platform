# Order Service - MAIP

## Overview
Manages order creation, processing, and fulfillment for merchant PoS purchases.

## Details
- **Package:** `com.maip.order`
- **Port:** 8086
- **Database:** `maip_{env}_order`
- **OpenTelemetry Name:** `maip-order-service`

## Kafka Topics

| Topic | Type | Description |
|-------|------|-------------|
| maip.order.created | Producer | New order placed |
| maip.order.completed | Producer | Order fulfilled |
| maip.order.cancelled | Producer | Order cancelled |
| maip.quote.accepted | Consumer | Quote accepted, create order |
| maip.application.approved | Consumer | Application approved, enable ordering |

## API Endpoints

```
POST   /api/v1/orders                # Create order
GET    /api/v1/orders/{id}           # Get order by ID
GET    /api/v1/orders                # List orders (paginated)
PUT    /api/v1/orders/{id}/status    # Update order status
POST   /api/v1/orders/{id}/cancel    # Cancel order
GET    /api/v1/orders/customer/{customerId}  # Orders by customer
```

## Build & Run

```bash
cd backend/microservices/order-service

# Build
./mvnw clean verify

# Run locally
./mvnw spring-boot:run -pl order-service-impl \
  -Dspring.profiles.active=local
```

## Deploy to EKS

```bash
kubectl apply -f DevOps/Kubernetes/services/order-service/ -n maip-services
kubectl rollout status deployment/order-service -n maip-services
```

## Emergency Scaling (High Load)

```bash
# Check current lag
kafka-consumer-groups.sh --bootstrap-server $MAIP_KAFKA_BROKERS \
  --describe --group maip-order-service-consumer

# Scale up
kubectl scale deployment order-service -n maip-services --replicas=12

# Monitor recovery
watch -n 5 'kubectl get pods -n maip-services -l app=order-service'
```

## Key Entities

```java
@Entity
@Table(name = "orders")
public class Order {
    UUID id;
    UUID customerId;
    UUID quoteId;
    OrderStatus status;  // PENDING, PROCESSING, COMPLETED, CANCELLED
    BigDecimal subtotal;
    BigDecimal taxAmount;
    BigDecimal discountAmount;
    BigDecimal totalAmount;
    String shippingAddress;
    Instant createdAt;
    Instant completedAt;
}

@Entity
@Table(name = "order_items")
public class OrderItem {
    UUID id;
    UUID orderId;
    UUID productId;
    int quantity;
    BigDecimal unitPrice;
    BigDecimal lineTotal;
}
```

## Event Payload

```java
@Value @Builder
public class OrderCreatedEvent implements MaipEvent {
    String eventId = UUID.randomUUID().toString();
    String eventType = "maip.order.created";
    Instant timestamp = Instant.now();
    String correlationId;
    OrderPayload payload;
}

@Value @Builder
public class OrderPayload {
    UUID orderId;
    UUID customerId;
    UUID quoteId;
    BigDecimal totalAmount;
    int itemCount;
    OrderStatus status;
}
```

## Logs & Debugging

```bash
# Tail logs
kubectl logs -f -l app=order-service -n maip-services --tail=100

# Check specific pod
kubectl logs order-service-xxxxx -n maip-services

# Describe pod (troubleshooting)
kubectl describe pod -l app=order-service -n maip-services
```
